...
namespace {
const bool use_hash_fptr = false;
...
void CGOpenMPRuntimeGPU::emitWorkerLoop(CodeGenFunction &CGF,
  ...
  for (llvm::Function *W : Work) {
    ...
    llvm::Value *ID =
        (CGM.getTriple().isAMDGCN() && use_hash_fptr)
            ? Bld.CreateIntToPtr(
    ...
    if (CGM.getTriple().isAMDGCN() && use_hash_fptr) {
  ...
  if ((CGM.getTriple().getArch() != llvm::Triple::amdgcn) || !use_hash_fptr) {
