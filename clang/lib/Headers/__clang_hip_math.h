...
#define __DEVICE_NOST__ __attribute__((always_inline, nothrow))
#else // Special case to build c-only device function lib for FORTRAN
...
#else  // NOT C++, c openmp compilation
#define __DEVICE__ static __attribute__((always_inline, nothrow))
#define __DEVICE_NOCE__ __DEVICE__
#endif
#endif

#else // not openmp, so this is for clang-hip which is always c++
      // could use __device__ here instead of __attribute__((device))
...
__DEVICE__
float log2i(int __x) { return __ocml_log2_f64((double) __x); }
